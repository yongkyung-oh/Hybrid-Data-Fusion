...
class PredictionModel(nn.Module):
    def __init__(self, seq_features=22, tab_features=22, emb_dim=32, hidden=128, num_class=10):
        ...
        feature_dim = tab_features+num_class
        self.feature_net = nn.Linear(tab_features,tab_features)

        self.q_ln = nn.Linear(feature_dim,emb_dim)
        self.k_ln = nn.Linear(feature_dim,emb_dim)
        self.v_ln = nn.Linear(feature_dim,emb_dim)
        self.self_attention_layer = nn.MultiheadAttention(embed_dim=emb_dim, num_heads=2, batch_first=True, dropout=0.2)
    ...
    def forward(self, feature, pred):
        ...
        x = torch.cat([feature, pred.log()], axis=1)
        x = x.unsqueeze(1)
        ...
        k = self.k_ln(x)
        v = self.v_ln(x)
        attn, _ = self.self_attention_layer(q,k,v)
        
        xx = self.emb(x)
        xx = xx+attn
        
        # mlp model
        out = self.mlp(xx.squeeze(1))
        return out
...
for user_id in user_list:
    if not os.path.exists('out_mme/{}'.format(user_id)):
        os.mkdir('out_mme/{}'.format(user_id))
    ...
    for k in range(10):
        ...
        for j in range(5):
            ...
            # with open(os.path.join('out_mme', user_id, '{}_{}'.format(modelname, k)), 'wb') as f:
            ...
            with open(os.path.join('out_mme', user_id, '{}_{}_{}'.format(modelname, k, j)), 'wb') as f:
                pickle.dump([y_test, y_pred, y_pred_proba, y_true_mm, y_pred_mm, y_pred_proba_mm], f)
