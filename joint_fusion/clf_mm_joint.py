...
class MM_Dataset(Dataset):
    def __init__(self, tab_all, seq_all, true_all):
    ...
    def __getitem__(self, idx):
        ...
        seq_all = np.stack(self.seq_all[idx]).T
        seq_all = torch.FloatTensor(seq_all.astype(float))
        true_all = self.true_all[idx]
                
        sample = {'target': true_all, 'seq': seq_all, 'feature': tab_features}
        return sample
...
class PredictionModel(nn.Module):
    def __init__(self, seq_features=22, tab_features=22, emb_dim=32, hidden=128, num_class=10):
        ...
        feature_dim = seq_features+tab_features
        self.feature_net = nn.Linear(tab_features,tab_features)
        self.lstm = nn.LSTM(seq_features,seq_features)
        self.pool = nn.AvgPool1d(60)
        self.emb = nn.Linear(feature_dim,emb_dim)
        self.mlp = nn.Sequential(nn.Linear(emb_dim,hidden),
                                 nn.ReLU(), nn.Dropout(0.2),
                                 nn.Linear(hidden,num_class))
        
    def forward(self, feature, seq):
        feature = self.feature_net(feature)
        out, _ = self.lstm(seq)
        seq = self.pool(out.permute(0,2,1)).squeeze()
        
        x = torch.cat([feature, seq], axis=1)
        x = self.emb(x)
        out = self.mlp(x)
        return out
...
def train(model, optimizer, criterion, train_iter):
    ...
    for batch in train_iter:
        ...
        logit = model(batch['feature'].to(device), batch['seq'].to(device))
...
for user_id in user_list:
    ...
    for k in range(10):
        ...
        train_dataset = MM_Dataset(tab_train, X_train, y_train_emb)
        test_dataset = MM_Dataset(tab_test, X_test, y_test_emb)
        ...
        # train model
        model = PredictionModel(num_class=max(y_train_emb)+1)
        model.to(device)
        ...
        with open(os.path.join('out_clf', user_id, '{}_{}'.format('lstm', k)), 'wb') as f:
            pickle.dump([y_true_mm, y_pred_mm, y_logit_mm], f)

        print(user_id, 'lstm', accuracy_score(y_true_mm, y_pred_mm), f1_score(y_true_mm, y_pred_mm, average='weighted'))
